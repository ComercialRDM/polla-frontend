...
export default function ListaPronosticos() {
    ...
    return (
        <div className="w-full max-w-md px-6 mt-6 relative z-10">
            <div className="rounded-2xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-slate-900/60 shadow-sm dark:shadow-none backdrop-blur-lg p-4">
                <p className="text-zinc-900 dark:text-white font-bold text-sm mb-1 text-center">
                    🔥 ¡{pronosticos.length} {pronosticos.length === 1 ? 'persona ya hizo' : 'personas ya hicieron'} su pronóstico!
                </p>
                <p className="text-zinc-500 dark:text-zinc-400 text-xs mb-3 text-center">
                ...
                <div className="max-h-56 overflow-y-auto pr-1 flex flex-col gap-1.5">
                    {pronosticos.map((p, i) => (
                        <div
                            key={i}
                            className="flex items-center justify-between rounded-xl bg-zinc-50 dark:bg-white/5 border border-zinc-100 dark:border-white/5 px-3 py-2"
                        >
                            <span className="text-zinc-700 dark:text-zinc-200 text-sm font-medium truncate">{p.nombre}</span>
                            <span className="font-scoreboard text-amber-500 dark:text-amber-400 text-sm flex-shrink-0 ml-2">
